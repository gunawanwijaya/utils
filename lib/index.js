...
const utils = () => {
    ...
    const merge = (a, b) => {
        ...
        b.forEach((c) => {
            Object.keys(c).forEach((p) => {
                a[p] = is(c[p], Object) ? merge(a[p] || {}, c[p]) : c[p]
            })
        })
     ...
     */
    const objectToQueryString = (obj, ptr) => Object.keys(obj)
        .map((key) => {
            tmp = ptr ? `${ptr}[${key}]` : key
            obj[key] = Array.isArray(obj[key]) ? `[${obj[key]}]` : obj[key]

            return typeof obj[key] === 'object'
                ? objectToQueryString(obj[key], tmp)
                : `${tmp}=${obj[key]}`
        })
        .join('&')
     ...
     * @returns {*} Object
     */
    const queryStringToObject = (str) => str.split('&')
        .map((v) => v.split('='))
        .reduce((obj, [k, v]) => {
    ...
    const browser = () => {
        ...
        /** # modified to es6 method from https://github.com/metatribal/xmlToJSON */ // eslint-disable-next-line
